...
table_fields = ("InsurerID", "TPAID", "ALNO", "ClaimNo", "PatientName", "AccountNo", "BeneficiaryBank_Name", "UTRNo",
                "BilledAmount", "SettledAmount", "TDS", "NetPayable", "Transactiondate", "DateofAdmission",
                "DateofDischarge", "unique_key", "mail_id", "hospital", "POLICYNO")
...
def mark_flag(flag, mid):
    with mysql.connector.connect(**conn_data) as con:
        ...
        q = "update settlement_mails set completed=%s where id=%s;" \
            "update utr_mails set completed='' where id=%s;"
        cur.execute(q, (flag, mid, mid))
        con.commit()
...
def ins_upd_data(mail_id, hospital, data_dict):
    pass
