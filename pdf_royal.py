...
try:
    ...
    data_dict = {'Sno': mid, 'HospitalID': 'inamdar', 'InsurerID': 'royal'}
    regex_dict = {
        'ALNO': [[r"(?<=Settlement of claim number:).*"], []], 'ClaimNo': [[r"(?<=Settlement of claim number:).*"], []],
        'PolicyNo': [[r"(?<=Policy Number).*(?=Patient Name)"], [':']], 'PatientName': [[r"(?<=Patient Name).*"], [':']],
        'DateofAdmission': [[r"(?<=Period of Hospitalization:).*(?=to)"], []],
        'DateofDischarge': [[r"\S+(?=\n*We are pleased to inform)"], []],
        'NetPayable': [[r"(?<=Amount Paid).*(?=\()"], [':', 'Rs']],
        'DateOfPayment': [[r"\S*(?=vide\n*Transaction Refernce number)"], []],
        'UTRNo': [[r"(?<=Transaction Refernce number:).*(?=towards)"], []],
        'BilledAmount': [[r"(?<=Billed Amount).*"], [':', 'Rs']], 'DiscountAmt': [[r"(?<=Discount).*"], [':', 'Rs']],
        'COPay': [[r"(?<=Co-payment).*"], [':', 'Rs']],
        'DeductibleAmt': [[r"(?<=Deductible).*"], [':', 'Rs']]
    }
