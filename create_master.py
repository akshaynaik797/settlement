...
try:
    ...
    sh1 = ['Sno', 'HospitalID', 'InsurerID', 'ALNO', 'ClaimNo', 'MemberID', 'PolicyNo', 'PatientName',
           'InsuranceCompany', 'AccountNo', 'BeneficiaryBank Name', 'Diagnosis', 'UTRNo', 'BilledAmount',
           'SettledAmount', 'TDS', 'NetPayable', 'DiscountAmt', 'COPay', 'PolicyHolder', 'IPNo', 'PrimaryBeneficiary',
           'EmployeeID', 'InsurerClaimNo', 'InsurerMemberID', 'TaxDeductedatSource', 'Netamount payment',
           'PaidbythePatient', 'ProrataBasis', 'PolicyExcessDeductible', 'BeneficiaryName',
           'BalanceSumInsuredBeforeClaim', 'NetPayable', 'BalanceSumInsuredAfterClaim', 'TDS%', 'Remarks', 'PaymentTo',
           'DateofAdmission', 'DateofDischarge', 'AmtPaidtoHospital', 'BillAmt', 'PayableAmt', 'SettledAmt',
           'SumInsured', 'ALAmount	Approved', 'Amount', 'HospitalAmount', 'AmountUtilised', 'FinalAmountSettled',
           'DateOfPayment', 'ServiceTax', 'TotalwithServiceTax', 'InsuredPerson', 'CorporateName', 'DeductibleAmt',
           'Transactiondate', 'LOCALAmount', 'ChequeDate', 'UHCApprovedHospitalAmt', 'InsurerApprovedHospitalAmt',
           'InsurerApprovedEmployeeAmt', 'PayableAmount', 'NEFTTransactionNumber', 'TransactionDate', 'CorporateName',
           'Claimed', 'PreHospitalisationPayableAmount', 'PostHospitalisationPayableAmount', 'AddonBenefit', 'Claimed',
           'Paid', 'BillAmount', 'PayableAmount(INR)', 'BillDate', 'BillNo', 'AmountSettled', 'ApprovedAmount', 'less',
           'Excess of Defined / Ailment Limit', 'policy deduction', 'Limit exceed deduction', 'non payable deduction',
           'Bill deduction', 'Other deduction']
    ...
    for i in range(0, 1):
        ...
        if ins == 'united':
            ...
            if s1.cell(row=1, column=1).value != None:
                ...
                for i in range(0, len(oe)):
                    ...
                    # print(det)
                    # print(iu,up,det-float(iu[0])-float(up[0]))
                    main_s1.cell(row=i + temp_row + 1, column=55).value = float(
        ...
        if ins == 'Paramount':
            ...
            if s1.cell(row=1, column=1).value != None:
                ...
                for i in range(0, len(b)):
                    ...
                    main_s1.cell(row=i + main_row_count + 1, column=17).value = float(ew[i]) - round(
                        float(ew[i]) / 10)  # netpayable
        ...
        l = {'01': 'Jan', '02': 'Feb', '03': 'Mar', '04': 'Apr', '05': 'May', '06': 'Jun', '07': 'Jul', '08': 'Aug',
    ...
    def deduction_none():
        ...
        for i in range(1, main_s2.max_row + 1):
            if main_s2.cell(row=i, column=8).value == None:
                ...
                mol.append(i - kol)
                kol += 1
            elif main_s2.cell(row=i, column=8).value == 0 or main_s2.cell(row=i,
                                                                          column=8).value == 0.00 or main_s2.cell(row=i,
                                                                                                                  column=5).value == 'Total' or main_s2.cell(
                    row=i, column=5).value == 'Final Approved Amount' or str(
                    main_s2.cell(row=i, column=8).value) == '' or main_s2.cell(row=i,
                                                                               column=5).value == 'Total Payble Amount' or main_s2.cell(
                    row=i, column=5).value == 'Miscellaneous':
                # main_s2.delete_rows(i, 1)
                mol.append(i - kol)
                kol += 1
    ...
    dg = ['Final\xa0Approved\xa0Amount', 'Total', 'Total Payble Amount', 'Total Payble Amount.',
    ...
    for k_num in range(0, len(temp)):
        ...
        if i == None and l_cat == None:
            dict_cat[temp_i] = ''
        elif i.lower().find('co-pay') != -1 or i.lower().find('copay') != -1 or i.lower().find(
                'co pay') != -1 or i.lower().find('co-payment') != -1:
            dict_cat[temp_i] = '1'

        elif i.lower().find('nme') != -1 or i.lower().find('non-consumable') != -1 or i.lower().find(
                'non payable') != -1 or i.lower().find('not payable') != -1:
            dict_cat[temp_i] = '5'

        elif i.lower().find('disc') != -1 and i.lower().find('discharge') == -1:
            dict_cat[temp_i] = '6'

        elif i.lower().find('per soc') != -1 or i.lower().find('tariff') != -1 or i.lower().find(
                'do not collect') != -1 or i.lower().find('not to be collect') != -1 or i.lower().find(
                'not to collect') != -1 or i.lower().find('as per mou') != -1 or i.lower().find(
                'as per ppn') != -1 or i.lower().find('as per mini soc') != -1 or i.lower().find(
                'as per hospital') != -1:
            dict_cat[temp_i] = '7'


        elif i.lower().find('as per patient') != -1 or i.lower().find('as per policy') != -1 or i.lower().find(
                'as per authorization') != -1 or i.lower().find('excess room') != -1:
            dict_cat[temp_i] = '2'

        elif i.lower().find('paid by patient') != -1 or i.lower().find('room') != -1:
            if l_cat != None:
                if l_cat.lower().find('package') != -1 or l_cat.lower().find('pkg') != -1 or l_cat.lower().find(
                        'icu') != -1 or l_cat.lower().find('hospital') != -1 or l_cat.lower().find(
                        'consultant') != -1 or l_cat.lower().find('room') != -1:
                    dict_cat[temp_i] = '2'
                else:
                    dict_cat[temp_i] = '5'
            else:
        ...
        elif i.lower().find('member paid') != -1 or i.lower().find('admin') != -1 or i.lower().find(
                'micro') != -1 or i.lower().find('casu') != -1 or i.lower().find('TPA') != -1 or i.lower().find(
                'payable') != -1 and i.lower().find('not') != -1 or i.lower().find('non') != -1:
            dict_cat[temp_i] = '5'

        elif i.lower().find('non') != -1 and i.lower().find('med') != -1 or i.lower().find('adm') != -1:
            dict_cat[temp_i] = '5'

        elif i.lower().find('exhausted') != -1 or i.lower().find('gipsa') != -1 or i.lower().find(
                'ppn') != -1 or i.lower().find('pkg') != -1 or i.lower().find('over') != -1 or i.lower().find(
                'excess') != -1 or i.lower().find('room') != -1 or i.lower().find('limit') != -1 or i.lower().find(
                'exceed') != -1:
            dict_cat[temp_i] = '3'
        ...
        if main_s1.cell(row=i, column=4).value in d_ccn:
            ...
            if main_s1.cell(row=i, column=19).value == None:
                ...
                if main_s1.cell(row=i, column=19).value is not None:
                    main_s1.cell(row=i, column=19).value = float(main_s1.cell(row=i, column=19).value) + cop[
                        main_s1.cell(row=i, column=4).value]
            #####################################################akshay
            ...
            if main_s1.cell(row=i, column=55).value is not None:
                main_s1.cell(row=i, column=55).value = float(main_s1.cell(row=i, column=55).value) - cop[
                    main_s1.cell(row=i, column=4).value]
            #####################################################akshayend
            if main_s1.cell(row=i, column=18).value == None:
                main_s1.cell(row=i, column=18).value = dis[main_s1.cell(row=i, column=4).value]

            elif main_s1.cell(row=i, column=18).value != dis[main_s1.cell(row=i, column=4).value] and main_s1.cell(
                    row=i, column=3).value != 'hdfc':
                #####################################################akshay
                ...
                if main_s1.cell(row=i, column=18).value is not None:
                    main_s1.cell(row=i, column=18).value = float(main_s1.cell(row=i, column=18).value) + dis[
                        main_s1.cell(row=i, column=4).value]
            #####################################################akshayend

            if main_s1.cell(row=i, column=3).value != 'hdfc':
                if main_s1.cell(row=i, column=55).value is not None:
                    main_s1.cell(row=i, column=55).value = float(main_s1.cell(row=i, column=55).value) - dis[
                        main_s1.cell(row=i, column=4).value]
            ...
            if main_s1.cell(row=i, column=16).value == None:
                ...
                if main_s1.cell(row=i, column=16).value is not None:
                    main_s1.cell(row=i, column=16).value = float(main_s1.cell(row=i, column=16).value) + Tax[
                        main_s1.cell(row=i, column=4).value]
            ##################################################akshayend
            ...
            if main_s1.cell(row=i, column=19).value == None:
                ...
                if main_s1.cell(row=i, column=19).value is not None:
                    main_s1.cell(row=i, column=19).value = float(main_s1.cell(row=i, column=19).value) + cop[
                        main_s1.cell(row=i, column=5).value]
            ##################################################akshayend
            ##################################################akshay
            ...
            if main_s1.cell(row=i, column=18).value == None:
            ...
            elif main_s1.cell(row=i, column=18).value != dis[main_s1.cell(row=i, column=5).value] and main_s1.cell(
                    row=i, column=3).value != 'hdfc':
                ##############################ak
                ...
                if main_s1.cell(row=i, column=18) is not None:
                    main_s1.cell(row=i, column=18).value = float(
                        str(main_s1.cell(row=i, column=18).value).replace(',', '').strip()) + dis[
                                                               main_s1.cell(row=i, column=5).value]
            ##############################akend
            if main_s1.cell(row=i, column=3).value != 'hdfc':
                if main_s1.cell(row=i, column=55).value is not None:
    ...
    ins_id = ['', 'aditya_birla', 'bajaj', 'apollo_munich', 'fgh', 'fhpl', 'health_india', 'health_hertige',
              'icici_lombard', 'MDINDIA', 'Medi_Assist', 'Paramount', 'religare', 'united', 'hdfc', 'vidal',
              'universal_sompo', 'vipul', 'Ease_West', 'Good_health', 'medsave', 'raksha', 'reliance',
              'health_insurance']
    t_mr = len(ins_id)
